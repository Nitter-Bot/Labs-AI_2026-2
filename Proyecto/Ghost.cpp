...
void Ghost::update(unsigned char i_level, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map, Ghost& i_ghost_0, Pacman& i_pacman, std::array<Ghost, 4>& all_ghosts)
{
	//Can the gohst move?
	bool move = 0;
	...
	update_target(i_pacman.get_direction(), i_ghost_0.get_position(), i_pacman.get_position(), i_map, all_ghosts);
...
void Ghost::update_target(unsigned char i_pacman_direction, const Position& i_ghost_0_position, const Position& i_pacman_position, const std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map, std::array<Ghost, 4>& all_ghosts)
{
